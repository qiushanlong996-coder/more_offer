"""QR code login for Xiaohongshu - Integrated with Web-Rooter.
...
Integration with Web-Rooter:
- Saves cookies to both local storage AND auth_profiles
- Uses Web-Rooter's cookie sync system
- Integrates with wr cookie command
"""
...
from .cookies import save_cookies, save_cookies_to_auth_profiles
...
def _browser_assisted_qrcode_login(
    *,
    ...
    with Camoufox(headless=False) as browser:
        ...
        # Save to both local storage and auth profiles (Web-Rooter integration)
        save_cookies(cookies)
        save_cookies_to_auth_profiles(cookies)
...
def qrcode_login(
    *,
    ...
    Web-Rooter Integration:
    - Saves cookies to ~/.web-rooter/xhs_cookies.json
    - Also saves to ~/.web-rooter/login_profiles.json (auth_profiles)
    - Compatible with `wr cookie xiaohongshu` workflow
    """
